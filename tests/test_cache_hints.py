...
from srgssr_mcp._app import CACHE_HINTS, LIST_CACHE_TTL_MS, mcp
...
async def test_die_werkzeugliste_traegt_die_ttl() -> None:
    ...
    assert result.ttl_ms == LIST_CACHE_TTL_MS, (
        f"tools/list antwortete mit ttlMs={result.ttl_ms}; bei 0 listet jeder Client bei jeder Verbindung neu auf"
    )
...
async def test_der_inhalt_einer_ressource_traegt_keinen_frischehinweis() -> None:
    ...
    """
    async with Client(mcp) as client:
        result = await client.read_resource("epg://srf/{channel_id}/{date}")
