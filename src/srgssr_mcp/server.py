...
# The thin-wrapper tools above each cover one upstream endpoint and return
# atomic responses. Aggregation tools fan out across multiple endpoints in
# parallel via :func:`asyncio.gather` and merge the results, so callers don't
# have to chain three roundtrips for cross-domain queries (e.g. "what's the
# weather and what's on TV in Zurich tonight?"). When one upstream fails the
# other section is still rendered so the response degrades gracefully.
